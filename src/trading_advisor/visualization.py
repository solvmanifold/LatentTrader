...
def create_combined_visualization(
    ...
    ticker: str,
    score: float,
    score_indicators: Dict[str, Any],
    chart_indicators: Dict[str, Any],
    output_dir: str = "output/charts"
) -> str:
    """
    Create a combined visualization with the score breakdown on top and the price/indicator chart below.
    Args:
        df: DataFrame with OHLCV data
        ticker: Stock ticker symbol
        score: Total technical score
        score_indicators: Dictionary for score breakdown (contributions, values)
        chart_indicators: Dictionary for price chart (usually score_details)
        output_dir: Directory to save the chart
    Returns:
        Path to the saved HTML file
    """
    ...
    # Row 1, col 1: indicator (domain)
    # Row 1, col 2: bar (xy)
    # Row 2, col 1: table (table)
    # Row 2, col 2: None
    # Row 3, col 1: price/volume/RSI (xy, colspan 2)
    # Row 3, col 2: None
    fig = make_subplots(
        rows=3, cols=2,
        specs=[
        ...
        subplot_titles=("Overall Score", "Component Breakdown", "Detailed Analysis", f"{ticker} Price/Volume/RSI")
    )

    # --- Score breakdown (reuse logic from create_score_breakdown) ---
    # Gauge
    fig.add_trace(
        go.Indicator(
            mode="gauge+number",
    ...
    # Bar
    score_components = {
        'RSI': score_indicators.get('rsi_score', 0),
        'Bollinger Bands': score_indicators.get('bb_score', 0),
        'MACD': score_indicators.get('macd_score', 0),
        'Moving Averages': score_indicators.get('ma_score', 0),
        'Volume': score_indicators.get('volume_score', 0),
        'Analyst Targets': score_indicators.get('analyst_targets_score', 0),
    }
    def get_bar_color(v):
        if v > 0:
            return 'rgba(50, 171, 96, 0.7)'
        elif v < 0:
            return 'rgba(219, 64, 82, 0.7)'
        else:
    ...
    # Table
    analyst_target = score_indicators.get('analyst_target', 0)
    target_upside = score_indicators.get('target_upside', 0)
    analyst_range = ''
    if 'analyst_target_low' in score_indicators and 'analyst_target_high' in score_indicators:
        analyst_range = f", range: ${score_indicators['analyst_target_low']:.0f}–${score_indicators['analyst_target_high']:.0f}"
    analyst_targets_str = (
    ...
    details = {
        ...
        'BB Position': f"{score_indicators.get('bb_position', 0):.1f}% ({'Upper' if score_indicators.get('bb_position', 0) > 80 else 'Lower' if score_indicators.get('bb_position', 0) < 20 else 'Middle'})",
        'MA Trend': score_indicators.get('ma_trend', 'Neutral'),
    ...
    # --- Price/indicator chart (reuse logic from create_stock_chart) ---
    # All traces go in row=3, col=1
    # Candlestick
    fig.add_trace(
        go.Candlestick(
            x=df.index,
    ...
    concise_text = f"Last: ${score_indicators.get('last_price', 0):.2f} | Target: ${score_indicators.get('analyst_target', 0):.2f} ({score_indicators.get('target_upside', 0):+.1f}%)"
    score_blurb = (
        "<b>Technical Score:</b> The score (0–10) is a weighted sum of signals from RSI, MACD, Bollinger Bands, Moving Averages, Volume, and Analyst Targets.<br>"
        "Each bar shows the contribution of a component to the total score.<br>"
        "<span style='font-size:10px'><b>How each score is computed:</b><br>"
    ...
    fig.update_layout(
        title={
            'text': f"{ticker} Technical Analysis: Score & Chart",
            'x': 0.5,
            'xanchor': 'center'
        },
        height=1200,
    ...
    # Create output directory if it doesn't exist
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, f"{ticker}_combined.html")
    fig.write_html(output_path)
    return output_path 