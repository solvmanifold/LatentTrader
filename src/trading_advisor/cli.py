...
def generate_classification_dataset(
    tickers: str = typer.Option("all", help="Comma-separated tickers, path to file, or 'all' for all tickers"),
    start_date: str = typer.Option("2020-06-04", help="Start date for dataset generation (YYYY-MM-DD)"),
    end_date: str = typer.Option("2025-06-04", help="End date for dataset generation (YYYY-MM-DD)"),
    target_days: int = typer.Option(5, help="Number of days to look ahead for labels"),
    target_return: float = typer.Option(0.02, help="Target return threshold for labels"),
    ...
    min_samples: int = typer.Option(10, help="Minimum number of samples required for a ticker"),
    ...
    from trading_advisor.models.dataset import DatasetGenerator, remove_unnecessary_features
    from trading_advisor.data import load_tickers
    import shutil
    from pathlib import Path
    import json
    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), BarColumn(), TaskProgressColumn()) as progress:
        ...
        for split_name, split_data in datasets.items():
            ...
            for dataset_name, data in split_data.items():
                if data.empty:
                    logger.warning(f"Empty dataset {split_name}_{dataset_name}")
                    continue
                    
                # Verify label column exists
                if 'label' not in data.columns:
                    logger.error(f"Missing label column in {split_name}_{dataset_name}")
                    continue
                
                # Remove unnecessary features
                data = remove_unnecessary_features(data)
                
                # Save feature mappings for the first split's training data
                if split_name == 'split_0' and dataset_name == 'train':
                    mappings = save_feature_mappings(data, Path(output_dir))
                
                # Apply mappings to categorical features
                data_mapped = apply_feature_mappings(data, mappings)
                
                # Save the processed dataset
                output_path = Path(output_dir) / split_name / f"{dataset_name}.parquet"
                output_path.parent.mkdir(parents=True, exist_ok=True)
                data_mapped.to_parquet(output_path)
                
                # Collect statistics for README
                label_dist = data_mapped['label'].value_counts(normalize=True)
                split_stats.append({
                    'split_num': split_num,
                    'set': dataset_name,
                    'samples': len(data_mapped),
                    'positive_pct': label_dist.get(1, 0),
                    'features': len(data_mapped.columns)
                })
                
                # Print validation info
                logger.info(f"\nValidating {split_name}_{dataset_name} dataset:")
                logger.info(f"Sample count: {len(data_mapped)}")
                
                # Print feature counts
                logger.info("\nFeature counts:")
                logger.info(f"Total features: {len(data_mapped.columns)}")
                logger.info(f"Ticker features: {len([col for col in data_mapped.columns if not col.endswith(('_market', '_sector'))])}")
                logger.info(f"Market features: {len([col for col in data_mapped.columns if col.endswith('_market')])}")
                logger.info(f"Sector features: {len([col for col in data_mapped.columns if col.endswith('_sector')])}")
...
- `split_0/` through `split_N/`: Each split contains train, validation, and test sets
...
- `1` (Positive): The stock's price increases by at least {target_return*100:.1f}% within {target_days} trading days
- `0` (Negative): The stock's price does not increase by {target_return*100:.1f}% within {target_days} trading days
...
- Prediction Window: {target_days} days
...
"""
    ...
    stats_df = stats_df.sort_values(['split_num', 'set'])
    
    for split_num in sorted(stats_df['split_num'].unique()):
        split_data = stats_df[stats_df['split_num'] == split_num]
        readme_content += f"\n#### Split {split_num}\n"
        for _, row in split_data.iterrows():
            readme_content += f"- {row['set'].title()} Set:\n"
            readme_content += f"  - Samples: {row['samples']}\n"
            readme_content += f"  - Positive Labels: {row['positive_pct']:.1%}\n"
            readme_content += f"  - Features: {row['features']}\n"
...
split_dir = Path("data/ml_datasets/split_0")
...
"""
    ...
    typer.echo(f"- Generated {len(datasets)} splits")
